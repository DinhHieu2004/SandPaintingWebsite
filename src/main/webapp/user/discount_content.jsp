    ...
    <div class="container_content">
      ...
      <div class="filter-section col-3">
        ...
        <form id="filterForm" method="GET" action="discount_content">
          ...
          <!-- Đánh giá tốt -->
          <div class="form-check mb-2">
            <input class="form-check-input" type="checkbox" id="sortByRating" name="sort" value="rating"
                   <c:if test="${param.sort eq 'rating'}">checked</c:if>>
            <label class="form-check-label" for="sortByRating">
              Đánh giá tốt nhất <i class="fas fa-star text-warning"></i>
            </label>
          </div>

          <!-- Ngày -->
          <div class="filter-group filter-latest-products card p-3">
            <h6 class="mb-3">Ngày</h6>
      ...
      <!-- Danh sách tác phẩm -->
      <div class="col-9">
        <div class="row g-4 g-2" id="artworkGallery">
          <c:forEach var="p" items="${data}">
            <div class="col-6 col-md-3">
              <div class="card artwork-card h-100" style="height: 380px;">
                <a href="painting-detail?pid=${p.id}" class="card-link"></a>
                <img src="${pageContext.request.contextPath}/${p.imageUrl}"
                     class="card-img-top artwork-image"
                     alt="${p.title}" style="width: 100%; height:180px;">
                <div class="card-body">
  ...
  <nav aria-label="Page navigation" class="mt-4">
    <ul class="pagination justify-content-center">
      <c:if test="${currentPage > 1}">
        <li class="page-item">
          <a class="page-link" href="discount_content?id=${discount.id}&page=${currentPage - 1}">&laquo;</a>
        </li>
      </c:if>
      <c:forEach var="i" begin="1" end="${totalPages}">
        <li class="page-item ${i == currentPage ? 'active' : ''}">
          <a class="page-link" href="discount_content?id=${discount.id}&page=${i}">${i}</a>
        </li>
      </c:forEach>
